= Knife vSphere for OVH Private Cloud PCC. 

= DESCRIPTION:

Un Plugin Opscode Knife permettant une communication avec l'API Vmware Vsphere.

C'est juste un fork du projet et j'ai renommé les verbes. Si les users OVH ont une idée sur des potentiels features ils peuvent fork et pourquoi pas faire une pull-request, je ferais le merge. L'intérêt et d'avoir un repo pour les users OVH et si on veut bricoler ou le faire évoluer a notre sauce on peut.

Rbvmomi, le layer ruby gerant la partie SOAP est compatible vSphere 5.0 API. 

https://www.vmware.com/support/developer/vc-sdk/visdk41pubs/ApiReference/index.html

this is an Opscode Knife plugin to interact with VMware's vSphere API. This plugin currently supports the following:
...
For initial development, the plugin targets all communication at a vCenter instance rather than at specific hosts. Only named user authentication is currently supported; you can add the credentials to your <tt>knife.rb</tt> file:
    ...
    knife[:ovh_cloud_host] = "vcenter-hostname"
    knife[:ovh_cloud_user] = "privileged username"
    knife[:ovh_cloud_pass] = "your password"
    knife[:ovh_cloud_dc] = "your-datacenter"
...
== knife ovh cloud vm list [-r, --recursive [--only-folders]]
...
== knife ovh cloud vm state [-s STATE, --state STATE] [-w PORT, --wait-port PORT] [-g, --shutdown] [-r, --recursive]
...
== knife ovh cloud vm clone <new vm name> --template <source template name> --cspec <customization_spec>
   ...
   --cplugin CUST_PLUGIN_PATH - Path to plugin that implements OvhCloudPlugin.customize_clone_spec and/or OvhCloudPlugin.reconfig_vm
